...
λ = Λ -> 6.6e-8*(101315./Λ.p)*(Λ.t/293.15)
...
vtoz = (Λ,v) -> @. Λ.qsh/(2.0π*Λ.l*v)*log(Λ.r2/Λ.r1)
f = (Λ,i,z,di) -> @. i.*ec.*cc(Λ,di)./(3.0π.*η(Λ).*z)
converge = (f,g) -> maximum(abs.(1.-f./g).^2.) < 1e-20
...
function setupSMPS(Λ, v1, v2, tscan, tc)
   ...
   global Ve = reverse(logspace(log10(v1), log10(v2), bins+1))  # Voltage bin-edges
...
function setupDMA(Λ, z1, z2, bins)
	global Tc = getTc(Λ)
	global Ze = logspace(log10(z1), log10(z2), bins+1)
